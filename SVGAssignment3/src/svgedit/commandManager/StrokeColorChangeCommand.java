...
public class StrokeColorChangeCommand implements CommandInterface{
	...
	public StrokeColorChangeCommand(View view,SVGPaint paint){
		elements.clear();
		paints.clear();
        this.view=view;
        this.paint=paint;
	...
	public void execute(){
        for (SVGElement elem : view.getSelectedElements()) {
            if (elem instanceof SVGStylable) {
            	...
            	paints.add(((SVGStylable) elem).getStroke().getRGBColor());
            	//paints.add(prePaint);
...
*/
		for(int i = 0; i<elements.size();i++){
			...
			((SVGStylable)elements.get(i)).getStroke().setValueFromPaint(svgPaint);
		}
		view.repaint();
	}
}